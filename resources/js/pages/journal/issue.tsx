...
export default function IssueDetail({ issue }: IssueProps) {
    return (
        <PublicLayout>
            <Head title={`Vol ${issue.volume_id} No ${issue.number} (${issue.year})`} />

            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div className="mb-8">
                    <Link href={route('journal.archives')} className="flex items-center gap-2 text-primary hover:underline mb-6 text-sm font-medium">
                        <ChevronLeft className="w-4 h-4" />
                        Back to Archives
                    </Link>
                    <div className="border-b pb-8">
                        <h1 className="text-4xl font-serif font-bold text-primary mb-2">
                            Vol. {issue.volume_id} No. {issue.number} ({issue.year})
                        </h1>
                        <p className="text-xl text-muted-foreground">
                            {issue.title || 'Table of Contents'}
                        </p>
                    </div>
                </div>
